...
class SchoolManage extends Component {
  ...
  courseColumns = [
    {
      ...
      key: 'name',
      render: (text, item) => (
        <a onClick={() => router.push(`/course/${item.school.id}/${item.id}`)}>{text}</a>
      ),
    },
    {
      title: '创建人',
    ...
    },
    {
      ...
      render: (_, item) => (
        <div>
          ...
          {moment(item.end_time).format('YYYY-MM-DD HH:mm:ss')}
        </div>
      ),
    },
    {
      ...
      render: (_, item) => (
        <div>
          <a style={{ marginRight: '8px' }} onClick={() => this.showEditModal('course', item)}>
            编辑
          </a>
          <a style={{ color: 'red' }} onClick={() => this.handleCourseDel(item)}>
            删除
          </a>
        </div>
      ),
  ...
  studentColumns = [
    ...
    // },
    {
      ...
      render: (_, item) => (
        <div>
          <a style={{ marginRight: '8px' }} onClick={() => this.showEditModal('student', item)}>
            编辑
          </a>
          <a style={{ color: 'red' }} onClick={() => this.handleStudentDel(item)}>
            删除
          </a>
        </div>
      ),
  ...
  render() {
    ...
    const getModalContent = () => {
      ...
      return (
        <Form {...this.formLayout} onSubmit={this.handleCourseSubmit}>
          ...
          <Form.Item label="课程时间">
            {getFieldDecorator('range-time-picker', {
              ...
              initialValue: current
                ? [moment(get(current, 'start_time', null)), moment(get(current, 'end_time', null))]
                : [],
        ...
        <Card bordered={false}>
          ...
          <div className={styles['add-btn-bar']}>
            ...
            <span className={styles['add-btn']}>
              <Button type="primary" size="small" onClick={() => this.showModal('course')}>
                创建
              </Button>
            </span>
          </div>
          ...
          <div className={styles['add-btn-bar']}>
            ...
            <span className={styles['add-btn']}>
              <Button type="primary" size="small" onClick={() => this.showModal('student')}>
                创建
              </Button>
            </span>
          </div>
